...
class Room extends Model implements HasDynamicValidation
{
    use HasFactory;
    ...
    public static function rules(string $operation = 'store'): array
    {
        ...
        if ($operation === 'update') {
            return [
                ...
                'has_private_wc' => ['sometimes', 'required', 'boolean']
            ];
        }

        return [
            'bed_amount' => ['required', 'integer', 'min:1'],
            'has_air_conditioning' => ['required', 'boolean'],
            'has_private_wc' => ['required', 'boolean']
        ];
    }
}
